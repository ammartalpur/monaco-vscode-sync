...
const FileTreeNode = ({ node, level, activeFileName, onFileClick }: { node: FileNode; level: number; activeFileName: string; onFileClick: (path: string) => void }) => {
    ...
    return (
      <div>
        <div 
          ...
          style={{ padding: `4px 16px 4px ${16 + level * 12}px`, cursor: "pointer", color: "#cccccc", fontSize: "13px", fontFamily: "sans-serif", display: "flex", alignItems: "center", gap: "6px" }}
        ...
        {isOpen && sortedChildren.map((child) => (
          <FileTreeNode key={child.path} node={child} level={level + 1} activeFileName={activeFileName} onFileClick={onFileClick} />
        ))}
      </div>
    );
  }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    room.on("broadcast", { event: "code-update" }, (payload) => {
      ...
      if (fileNameRef.current === "Waiting for VS Code..." || fileNameRef.current === incomingFileName) {
      ...
      if (editorRef.current && monacoRef.current) {
        decorationRef.current = editorRef.current.deltaDecorations(
          ...
          [{
              ...
              options: { className: "remote-cursor", hoverMessage: { value: "VS Code Host" } },
          }],
        );
      }
    });
  ...
  const handleEditorDidMount: OnMount = (editor, monaco) => {
    ...
    editor.onDidChangeCursorPosition((e) => {
      if (channelRef.current && fileNameRef.current !== "Waiting for VS Code...") {
  ...
  const requestFileOpen = (path: string) => {
    if (channelRef.current) {
        ...
        channelRef.current.send({ type: "broadcast", event: "open-file", payload: { path: path } });
    }
  };

  const handleRefreshTree = () => {
    if (channelRef.current) {
        channelRef.current.send({ type: "broadcast", event: "request-file-tree", payload: {} });
    }
  };
  ...
  return (
    <div style={{ height: "100vh", width: "100%", display: "flex", flexDirection: "column", boxSizing: "border-box" }}>
      ...
      <header style={{ padding: "12px 24px", backgroundColor: "#1e1e1e", color: "#cccccc", borderBottom: "1px solid #3c3c3c", fontFamily: "sans-serif", display: "flex", justifyContent: "space-between", alignItems: "center", boxSizing: "border-box", height: "50px", zIndex: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          <button 
            onClick={() => setIsSidebarOpen(prev => !prev)}
            style={{ background: "none", border: "none", color: "#cccccc", cursor: "pointer", fontSize: "18px", padding: 0, display: "flex", alignItems: "center" }}
      ...
      <div style={{ flex: 1, display: "flex", width: "100%", overflow: "hidden" }}>
        <div style={{ 
            ...
            transition: "width 0.3s ease, min-width 0.3s ease, opacity 0.2s ease, padding 0.3s ease",
            whiteSpace: "nowrap" 
        }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 16px 8px 16px" }}>
                <span style={{ color: "#858585", fontSize: "11px", fontWeight: "bold", textTransform: "uppercase", letterSpacing: "1px" }}>Explorer</span>
                <button onClick={handleRefreshTree} style={{ background: "none", border: "none", color: "#cccccc", cursor: "pointer", fontSize: "16px", padding: "0" }} title="Refresh File List">⟳</button>
            </div>
            <div style={{ paddingTop: "4px" }}>
                {sortedTopLevel.length === 0 ? (
                    <div style={{ padding: "10px 16px", fontSize: "12px", color: "#858585", fontStyle: "italic" }}>No files shared yet. Open a file in VS Code.</div>
                ) : (
                    sortedTopLevel.map((node) => (
                        <FileTreeNode key={node.path} node={node} level={0} activeFileName={fileName} onFileClick={requestFileOpen} />
                    ))
                )}
            </div>
        </div>
